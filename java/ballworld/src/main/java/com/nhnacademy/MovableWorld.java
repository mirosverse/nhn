    ...
    public void move() {
        if ((getMaxMoveCount() == 0) || (getMoveCount() < getMaxMoveCount())) {
            for (int i = 0; i < getBallCount(); i++) {
                Regionable object = getObject(i);
                if (object instanceof MovableBall) {
                    ((Movable) object).move();

                    if (object instanceof BoundedBall) {
                        for (int j = 0; j < getBallCount(); j++) {
                            Regionable otherBall = getObject(j);

                            if ((object != otherBall) && (object.getRegion().intersects(otherBall.getRegion()))) {
                                ((Bounded) object).bounce(otherBall);
                                // logger.info("ball({})와 ball({})이 충돌하였습니다.",
                                // object.getId(),otherBall.getId());
                            }
                        }
                    }
                    moveCount++;
